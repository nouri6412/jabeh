...
class Work_Hours {
	...
	public function get_label_for_preview_card() {
		...
		if ( in_array( $status, [ 'open', 'closing', 'open-all-day' ], true ) ) {
			return _x( 'باز', 'Preview Card: Work hours status', 'my-listing' );
		}

		if ( $status === 'appointment-only' ) {
			return _x( 'فقط با تعیین وقت قبلی', 'Preview Card: Work hours status', 'my-listing' );
		}

		return _x( 'بسته', 'Preview Card: Work hours status', 'my-listing' );
	}
